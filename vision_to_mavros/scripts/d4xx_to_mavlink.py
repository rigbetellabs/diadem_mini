...
def send_obstacle_distance_message():
    ...
    global last_obstacle_distance_sent_ms, obstacle_msg_count
    ...
    if angle_offset is None or increment_f is None:
        ...
        # Log the full array periodically (roughly once per second) so it's readable
        # instead of flooding the console at obstacle_distance_msg_hz (e.g. 15/sec)
        if obstacle_msg_count % int(max(obstacle_distance_msg_hz, 1)) == 0:
            progress("SENT #%d OBSTACLE_DISTANCE: %s" % (obstacle_msg_count, str(distances)))
